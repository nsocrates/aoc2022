...
Your puzzle answer was 12555527364986.
"""
...
puzzle_input = """Sensor at x=3907621, y=2895218: closest beacon is at x=3790542, y=2949630
...
class Solution:
    ...
    @staticmethod
    def get_tuning_frequency_with_y(y, intervals):
        frequency = 0
        for start, end in intervals:
            if frequency < start:
                return frequency * Solution.MULTIPLIER + y
            frequency = max(frequency, end + 1)
            if frequency > Solution.UPPER_BOUND:
                break
        return None

    def __init__(self, puzzle_input):
        self.sensor_report = self.parse_input(puzzle_input)

    def get_intervals_with_y(self, y):
        intervals = []
        for sx, sy, bx, by in self.sensor_report:
            ...
            lower_bound = sx - offset
            upper_bound = sx + offset
            intervals.append([lower_bound, upper_bound])
        return intervals

    def run(self):
        for y in range(self.UPPER_BOUND + 1):
            intervals = self.get_intervals_with_y(y)
            merged_intervals = self.merge_intervals(intervals)
            tuning_frequency = self.get_tuning_frequency_with_y(y, merged_intervals)
